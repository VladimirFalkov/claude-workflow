...
## Controversial decisions — use dialectic skill

Для спорных архитектурных решений, где есть реальный трейдофф (не очевидный выбор):

1. Используй skill `dialectic` — он запускает параллельный thesis/antithesis анализ и синтез.
2. Триггер: когда ты сам не уверен в выборе ИЛИ когда у тебя есть 2+ равноценных альтернативы.
3. Результат dialectic включается в раздел `## Rejected alternatives` архитектурного документа.

Примеры когда dialectic нужен: монолит vs микросервисы, синхронный vs асинхронный pipeline,
нормализация vs денормализация, REST vs GraphQL для конкретного use-case.

Примеры когда НЕ нужен: выбор очевиден из стека (Django есть — используем Django ORM),
решение продиктовано ограничением (нет Redis — нет async queue).
