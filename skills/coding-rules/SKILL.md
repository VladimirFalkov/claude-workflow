...
Приватные методы (`_name`) — по необходимости. Публичный API без docstring —
неявный контракт, который читатель должен угадывать по коду.

ХОРОШО:
```python
def calculate_area(polygon: Polygon) -> Decimal:
    """Вычислить площадь полигона в гектарах.

    Args:
        polygon: Геометрия в EPSG:4326.

    Returns:
        Площадь в гектарах, округлённая до 4 знаков.

    Raises:
        ValueError: Если SRID полигона не 4326.
    """
```

#### Inline-комментарии отвечают на «почему», не «что»

Код говорит «что». Комментарий объясняет «почему» — скрытое ограничение,
нетривиальный инвариант, обходной путь для конкретного бага.
...
ХОРОШО:
```python
offset += 1     # GeoJSON индексы с 1, а не с 0 — исторический артефакт формата
```
...
#### TODO только со ссылкой на BACKLOG

Голый `# TODO` исчезает из внимания через неделю. Ссылка на BACKLOG-NNN
даёт задаче адрес, приоритет и контекст.
...
ХОРОШО:
```python
# TODO: заменить на streaming API — see BACKLOG-042
# FIXME: N+1 на CompanyQuerySet.with_fields() — see BACKLOG-057
```

`# XXX:` — запрещён полностью: не несёт информации о статусе или плане.
...
|---|---|
| `datetime.now()` без tz | Naive datetime — тихие баги при сравнении |
| `float` для денег | Накапливающаяся ошибка округления |
| Bare `except:` | Перехватывает SystemExit, маскирует сбои |
| `from module import *` | Засоряет namespace, ломает статический анализ |
| `print()` в продакшн-коде | Нельзя фильтровать, структурировать, перенаправить |
| Hardcoded secrets в коде | Попадание в git-историю необратимо |
| `os.environ.get("KEY", "default")` для обязательных переменных | Тихий сбой при неправильной конфигурации |
| `# TODO` без BACKLOG-ссылки | Технический долг без адреса — исчезает |

---

## Related skills

- `testing-rules` — парная. Покрывает тот же Python-стек, но для тест-кода.
  Общие принципы (no magic strings, Decimal) формулируются в обоих, с разным фокусом.
- `coding-django` — расширяет `coding-rules` для Django ORM, views, services.
  Применяется если `CLAUDE.md` объявляет `backend: django`.
- `coding-fastapi` — расширяет для FastAPI, Pydantic v2, async patterns.
  Применяется если `backend: fastapi`.
- `coding-postgis` — GIS-специфика: SRID, ST_-функции, GiST индексы.
  Применяется если `database: postgis`.
